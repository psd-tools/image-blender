#cython: cdivision=True, emit_code_comments=False
...
#   http://wwwimages.adobe.com/content/dam/Adobe/en/devnet/pdf/pdfs/pdf_reference_1-7.pdf#page=520
#
...
# http://www.agner.org/optimize/optimizing_assembly.pdf#page=142, section 16.9
...
def dissolve(bytes imdata1, bytes imdata2):
    ...
    if length != len(imdata2):
        raise ValueError("Modes or sizes of images mismatch")
    if length & 3:
        raise ValueError("Both images must be in RGBA mode")

    cdef const UINT8 *pdata1 = imdata1
    cdef const UINT8 *pdata2 = imdata2
    cdef UINT8 *pres = <UINT8 *>malloc(length)
    cdef bytes res

    # Dummy seed. Original seed and RNG function are unknown.
    srand(314159265)
